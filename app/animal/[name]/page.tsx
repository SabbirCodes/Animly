      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            <ul className="list-disc list-inside text-gray-600 dark:text-gray-400">
              {Object.entries(animal.taxonomy).map(([key, value]) => (
                <li key={key}><span className="font-semibold">{key.replace(/_/g, ' ').toLowerCase().replace(/^./, str => str.toUpperCase())}:</span> {value}</li>
              ))}
            </ul>
          </div>
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600 dark:text-gray-400">{animal.locations.join(', ')}</p>
          </div>
